...
def train(config):
    ...
    # model.load_state_dict(torch.load("/usr/users/sdim/sdim_22/team-6-kaggle-challenge-deep-learning/model_logs/Baseline_UnetPlus/best_model.pt")) 
    model.to(device)
...
def test(config):
    ...
    model.load_state_dict(torch.load("model_logs/UnetPlus_1/best_model.pt"))
    model.to(device)
