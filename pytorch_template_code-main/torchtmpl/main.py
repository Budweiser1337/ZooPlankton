...
def train(config):
    ...
    model.load_state_dict(torch.load("/usr/users/sdim/sdim_22/team-6-kaggle-challenge-deep-learning/pytorch_template_code-main/model_logs/UnetPlus_6/best_model.pt")) 
    model.to(device)
...
def test(config):
    ...
    model.load_state_dict(torch.load("model_logs/UnetPlus_10/best_model.pt"))
    model.to(device)
